        ...
        children: [
          ...
          Row(
            children: [
              ...
              _buildCircleButton(Icons.code, () => _importThemeFromText(context)),
            ],
          ),
...
class _ColorPickerSheet extends StatefulWidget {
  ...
  const _ColorPickerSheet({required this.currentColor, required this.onColorSelected});
  @override
...
class _ColorPickerSheetState extends State<_ColorPickerSheet> {
  ...
  void _updateColor(Offset p, Size s) {
    ...
    setState(() { _currentSaturation = x; _currentBrightness = 1 - y; });
    widget.onColorSelected(HSLColor.fromAHSL(1, _currentHue, _currentSaturation, _currentBrightness).toColor());
  }

  @override
  void dispose() { _hexController.dispose(); super.dispose(); }
}
...
class _CustomThemeSheetState extends State<_CustomThemeSheet> {
    ...
    return Container(
      ...
      child: ListView(
        ...
        children: [
          ...
          _buildPicker(context, 'Background', _background, (c) => setState(() => _background = c)),
          _buildPicker(context, 'Surface', _surface, (c) => setState(() => _surface = c)),
          _buildPicker(context, 'Text', _text, (c) => setState(() => _text = c)),
          _buildPicker(context, 'Text Secondary', _textSecondary, (c) => setState(() => _textSecondary = c)),
          const SizedBox(height: 24),
          SizedBox(
  ...
  Widget _buildPicker(BuildContext context, String label, Color c, Function(Color) f) {
    final theme = Theme.of(context);
    return Padding(
  ...
  void _showColorPicker(Color current, Function(Color) onChanged) {
    final theme = Theme.of(context);
    showModalBottomSheet(context: context, backgroundColor: theme.colorScheme.surface, isScrollControlled: true, builder: (c) => _ColorPickerSheet(currentColor: current, onColorSelected: onChanged));
  }
}
...
class _ImportThemeSheetState extends State<_ImportThemeSheet> {
  ...
  void _apply() async {
    for (final line in _controller.text.split('\n')) {
      ...
      switch (k) {
        case 'primary': await widget.themeProvider.setCustomColor('primary', v); break;
        case 'secondary': await widget.themeProvider.setCustomColor('secondary', v); break;
        case 'background': await widget.themeProvider.setCustomColor('background', v); break;
        case 'surface': await widget.themeProvider.setCustomColor('surface', v); break;
        case 'text': await widget.themeProvider.setCustomColor('text', v); break;
        case 'textsecondary': await widget.themeProvider.setCustomColor('textsecondary', v); break;
      }
    }
    Navigator.pop(context);
  }
